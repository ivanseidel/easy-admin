...
function REST(options, objectToInstall){
	...
	*/
	self.update_x_editable = function(req, res, next){
	...
	// Mix in objects if requested
	if(objectToInstall)
		_.extend(objectToInstall, self);
	...
	self.app.route([
		...
		{method: 'post', 	path: self.basePath + '/post', 			callback: self.update_x_editable},
		// {method: 'get', 	path: self.basePath + '/post', 			callback: self.update_x_editable},
	...
	console.log('REST API created: '.green + self.basePath.cyan);
